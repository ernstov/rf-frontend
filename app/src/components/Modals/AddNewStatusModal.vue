    ...
    >
      ...
      >
        <div class="header flex justify-between">
          <h2 class="text-5xl mb-8">Add New Status</h2>
...
import { createStatusSchema } from "../../utils";
...
export default {
  ...
  methods: {
    async onSubmit() {
      try {
        await createStatusSchema.validate(this.form);
      } catch (error) {
      ...
      try {
        this.loading = true;
        const { data } = await StatusRepository.create(this.form);
        this.loading = false;
