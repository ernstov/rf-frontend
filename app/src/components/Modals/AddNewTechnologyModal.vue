    ...
    >
      ...
      >
        <div class="header flex justify-between">
          <h2 class="text-5xl mb-8">Add New Technology</h2>
...
import { createTechnologyTypeSchema } from "../../utils";
...
export default {
  ...
  methods: {
    async onSubmit() {
      try {
        await createTechnologyTypeSchema.validate(this.form);
      } catch (error) {
      ...
      try {
        this.loading = true;
        const { data } = await TechnologyRepository.create(this.form);
        this.loading = false;
