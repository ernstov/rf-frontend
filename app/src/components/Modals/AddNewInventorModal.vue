    ...
    >
      ...
      >
        <div class="header flex justify-between">
          <h2 class="text-5xl mb-8">Add New Inventor</h2>
...
import { createInventorSchema } from "../../utils";
...
export default {
  ...
  methods: {
    async onSubmit() {
      try {
        await createInventorSchema.validate(this.form);
      } catch (error) {
      ...
      try {
        this.loading = true;
        const { data } = await InventorRepository.create(this.form);
        this.loading = false;
        this.toast.success("Inventor Created");
